...
export const PostsList = ({ author, likedPostsUser }) => {
	...
	useEffect(() => {
		...
		if (searchTopicId === '0') {
			...
			if (author) {
				...
				setFilteredPosts(myPosts.filter((post) => post.title.toLowerCase().includes(searchTerm.toLowerCase())))
			} else {
				...
				if (likedPostsUser) {
					...
					setFilteredPosts(
						likedPosts.filter((post) => post.title.toLowerCase().includes(searchTerm.toLowerCase()))
					)
				} else {
					// otherwise, if there is no likedPostsUser
					// filter all posts by term
					setFilteredPosts(
						allPosts.filter((post) => post.title.toLowerCase().includes(searchTerm.toLowerCase()))
					)
				}
			}
		} else {
	...
	const renderPosts = () => {
		if (author) {
			...
			return filteredPosts.map((post) => {
				return <Post key={post.id} post={post} topics={allTopics} likes={allLikes} />
			})
		}
	}
